...
toc:
  ...
  - name: Single Query Long Context
    subsections:
      - name: Block Based Computation
      - name: Multihop Information Tracing
      - name: Dynamic Sparsity
      - name: Retrieval based Sparsity
        subsections:
          - name: How it works
...
Large Language Models (LLMs) have brought about a major change in artificial intelligence. They have transitioned in scope from being specialized research tools to common resources that drive the next generation of software. By increasing model parameters and training data, LLMs show new abilities in reasoning, code generation, and solving complex problems that were once considered unattainable. Scaling transformers effectively for long context applications uniquely poses a challenge. This is primarily because the self-attention mechanism has space and time complexity 

$$
\mathcal{O}\left(n^2\right)
$$

This quadratic bottleneck inhibits applications for long documents, high-resolution images, large codebases, etc. 

Sparsity emerges as an effective solution to this problem. Rather than relying on the N x N attention matrix, it utilizes an approximate or “sparse” version. The backbone of this approach is the idea that tokens do not require the entire context; they only need local context, and thus, most of the computation being carried out is wasteful.

{% raw %}{% include figure.liquid path="assets/img/2026-04-27-Sparsity/1.png" class="img-fluid" %}{% endraw %}
...
Although the sparsity pattern of an attention matrix is dynamic, it is observed to occur in patterns:

{% raw %}{% include figure.liquid path="assets/img/2026-04-27-Sparsity/2.png" class="img-fluid" %}{% endraw %}
...
Thus, MInference allows for different approaches to sparsity based on the pattern observed in the relevant attention matrix.

---

## Single Query Long Context

### Block Based Computation

Block-based methods partition the full N×N attention matrix into smaller, computationally feasible sub-matrices. Instead of all-to-all attention, computation is restricted to localized blocks of tokens. An example of this is sliding window attention (SWA), where each query token qi​ only attends to a fixed-size window of key tokens kj​ where ∣i−j∣≤w (for a window size w).

### Multihop Information Tracing

This local sparsity approach poses a challenge with information propagation. Direct computation of attention between two unadjacent tokens (e.g., qi​ and kj​ where ∣i−j∣≫w) is prohibited within a single layer. Consequently, information must be propagated across multiple layers, "hopping" from one block to the next. This multi-hop path can lead to signal attenuation and creates a propagation bottleneck, hindering the model's ability to effectively capture long-range dependencies.

{% raw %}{% include figure.liquid path="assets/img/2026-04-27-Sparsity/3.png" class="img-fluid" %}{% endraw %}

Star Attention largely mitigates this problem by adopting a query-centric global attention pattern. Each block is processed with local attention, but it also attends to a shared, trainable anchor block (i.e., the first block). This anchor acts as a global context summarizer, a hub in a star-graph topology, to which all blocks can write. A query token computes attention over two sets of keys: 1) within its local block, and 2) from the anchor block. This two-phase allows any query to access a summarized version of the global information and thus, the need for propagation through multilayer hopping is eliminated.

### Dynamic Sparsity

Unlike the static patterns of block-based methods, dynamic sparsity approaches generate content-aware, sparsity masks that are input-dependent. The specific attention pattern S is a function of the input tokens, S=f(Q,K,V). Therefore, the model learns the relevance of token interactions for specific inputs, rather than relying on a generalised pattern and allocates computation accordingly.

An implemented example of this is Tidal Attention. It works by dynamically adjusting the budget of tokens being attended to. This is implemented in the form of a learned threshold or gating mechanism applied to the attention score matrix A=QKT. After computing the full (or a subset of) scores, the model prunes any Aij​ score that falls below τ, which is a dynamically computed, sequence-specific threshold. This effectively focuses computation on only the qi​-kj​ pairs that have the highest relevance for that specific input.

### Retrieval based Sparsity

Retrieval-based methods radically expand the notion of context by performing attention calculations over a massive external corpus of key-value pairs, and not just the local sequence context. This decouples the context size from the computational window, allowing the model to theoretically access billions of tokens. The model can now theoretically access billions of tokens as the context size is no longer coupled to the computational window.

#### How it works

Retrieval Attention replaces the local K and V matrices with a top-k subset retrieved from an external datastore.

1. A query vector qi​ is generated from the input sequence.
2. This qi​ is used to query a large-scale, pre-indexed vector database.
3. This search is made feasible at inference time by the use of an Approximate Nearest Neighbor (ANN) index (e.g., HNSW, FAISS). These indices facilitate sub-linear time retrieval of the k vectors which are most similar to qi​ according to a given metric (e.g., Maximum Inner Product Search - MIPS).
4. The attention mechanism is then computed only over this small, retrieved set of highly relevant (k,v) pairs from the external corpus.

A critical failure of these methods, however, is the Out of Distribution (OOD) problem. It describes a mismatch in distribution between the query vectors and the key vectors stored in the index. This often happens because queries are generated from the final layers of a model, while keys may be generated from intermediate layers, different models, or during a separate indexing-time process. This divergence can cause the query and key vectors to occupy non-overlapping regions in the embedding space.

When a query qi​ is OOD from the indexed K distribution, it falls far from the data manifold on which the index was built and the ANN fails. This results in the retrieval of irrelevant nearest neighbors. Thus, the ANN attends to entirely useless information and the attention mechanism fails.

### Hardware Aware Methods

Computational speed is dependent not only on FLOPs but also the specific hardware architecture and memory access patterns. Unstructured sparsity patterns are inefficient on parallel processors as they lead to non-coalesced memory access and load imbalance. Hardware-aware methods design sparsity patterns in ways that are best suited to leverage the specific architecture.

**Hashattention**

It utilizes Locality-Sensitive Hashing (LSH) - a technique where high-dimensional vectors that are "close" (e.g., in angular similarity) are mapped to the same hash bucket with high probability. Queries and keys are hashed into multiple buckets. Attention computation is then restricted to only those qi​ and kj​ pairs that are in the same hash bucket. This bucketing converts the quadratic attention problem into a set of smaller dense matrix multiplications within each bucket. This is a compute pattern that is highly optimized for GPUs.


**Neighbourhood-Based Sparse Attention**

This is a general term that refers to methods that impose a highly structured and static sparsity pattern. The sliding window is the simplest form. Other examples include dilated (or strided) attention, which attends to tokens at fixed intervals (e.g., i±1,i±4,i±16), or fixed-block patterns. The key characteristic is the regularity of the pattern, which allows for pre-computation of indices, coalesced memory access, and efficient implementation using custom GPU kernels. This minimizes the overhead of indirect addressing that plagues unstructured sparsity.
While hardware-aware methods, such as HashAttention, optimize the computation of attention scores, they do not address a key problem. The memory required to store the Key-Value (KV) states grows linearly with sequence length. For long-context applications, this memory consumption eventually forces the system into Out-Of-Memory (OOM) errors, necessitating cache optimisations as well.
...
{% raw %}{% include figure.liquid path="assets/img/2026-04-27-Sparsity/4.png" class="img-fluid" %}{% endraw %}
...
Another strategy that has been explored is to prune the context during the prefilling stage based on contextual importance. The H20 framework introduces such a strategy. They employ a greedy policy that chooses the “Heavy Hitters” (H2​) along with the most recent tokens.

#### Heavy Hitters

Generally, it is observed that the attention scores of tokens follow a Power-Law distribution. The previous statement implies that a small subset of tokens contributes to the vast majority of the value when computing attention scores. These critical tokens are referred to as Heavy Hitters. Removing them causes significant performance degradation, whereas removing the other tokens does not. This allows a 20x cache reduction.

#### Eviction

The entire process can be formulated as a dynamic submodular problem, which involves calculating a normalised attention score for the ith token at the ith decoding step, based on the current cache content.

$$
o_i := D_i^{-1} \cdot \exp \left( Q_{i,*} (K_{S_i,*})^\top \right)
$$

$Q_{i,*}$ is the query vector for the current step.

$K_{S_i,*}$ is the sub-matrix of keys corresponding to the retained tokens in $S_i$.

$D_i$ is the normalisation scalar.

The eviction policy employed does not access future tokens at a certain decoding step, as in autoregressive cases, that is not the case. It purely relies on the local value computed at each step

Algorithm Logic: At each step i, the algorithm updates the set of retained tokens Si​ based on the previous set Si−1​.

Cache Not Full: If the number of tokens processed is less than the budget k, the current token is simply added to the cache: Si​←Si−1​∪{i}.

Cache Full: If the cache is full, the algorithm must evict one token to make room for the new one. It selects the token u that contributes the least to the accumulated attention score.


The decision at each timestep is defined as finding u, such that the remaining set maximizes the cumulative attention scores. Additionally, to maintain high throughput and low latency. To avoid swapping, the algorithm utilizes a preallocated memory block and a circular queue to manage the "recent tokens" portion of the cache, while maintaining heavy hitters based on their accumulated scores.

#### Theoretical Formulation

The writers also provided theoretical guarantees for their approach i.e, proving it was not just a heuristic. They do this by comparing their greedy algorithm with the optimal policy. The explanation is based on the following equation:

$$
f(\tilde{S}_i) \geq (1 - \alpha) \left(1 - \frac{1}{e}\right) \cdot \text{OPT}_i - \beta
$$

In intuitive terms, the above equation can be interpreted as the H20 policy achieves at least 63% of the performance of the optimal policy. Here’s how.

The (1-1/e) factor was introduced because the problem was defined as submodular, which in simple terms is “diminishing returns”. And the above constant is directly associated with all submodular problems.

(1-α) represents the dynamic penalty or the error accumulation over time. This term was introduced to account for the quality loss between decoding steps. Also, the error term was proven not to explode via Induction in the paper.

β (beta) is the additive error term. It accounts for the difference between the global attention score of a token and the approximate score H2O used to make its decision.

The optimal policy was defined as the following. The score one would get if one had infinite time and memory to test every possible combination of k tokens and pick the best one. 
