...
**A-Shape**
This structure combines detailed local context with sparse long-range signals. Tokens focus heavily on their immediate neighbors (forming the wide base of the "A") while simultaneously attending to a few specific "summary" tokens earlier in the context (forming the peak).

**Vertical-Slash (Strided/Dilated)**
This represents a fixed-stride approach. Instead of a contiguous local window, a token might attend to every $k$-th token (e.g., indices $i, i-100, i-200$). This "dilated" view allows the model to sample the entire history efficiently, creating a skeleton of long-range dependencies.
